...
InputSCOV("scovL2", cl::desc("<input scov file>"), cl::init(""), cl::Hidden);

static cl::opt<std::string>
InputUCOV("ucovL2", cl::desc("<input ucov file>"), cl::init(""), cl::Hidden);

static cl::opt<std::string>
InputLOG("logL2", cl::desc("<input log file>"), cl::init(""), cl::Hidden);

static cl::opt<double>
Threshold("use-asapL2", cl::desc("Fraction of dynamic checks to be preserved"), cl::init(-1.0));

bool DynPassL2::runOnModule(Module &m) {
    ...
    errs() << "DynPassL2 on "<<filename << ";" << Twine(InputSCOV).str().c_str() << "\n";
    ...
    fclose(fpp);
    // budgetAnalysis(m, fp_sc, fp_uc, reducedSC, TTIWP);
    }
...
bool DynPassL2::findPhiInst(Instruction *UC_Inst, Instruction *SC_Inst, uint64_t id1, uint64_t id2) {
...
void DynPassL2::optimizeCheckAway(Instruction *Inst) {
...
void DynPassL2::getAnalysisUsage(AnalysisUsage& AU) const {
...
char DynPassL2::ID = 0;
static RegisterPass<DynPassL2> X("DynPassL2",
