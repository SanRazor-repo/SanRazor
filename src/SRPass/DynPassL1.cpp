...
InputSCOV("scovL1", cl::desc("<input scov file>"), cl::init(""), cl::Hidden);

static cl::opt<std::string>
InputUCOV("ucovL1", cl::desc("<input ucov file>"), cl::init(""), cl::Hidden);

static cl::opt<std::string>
InputLOG("logL1", cl::desc("<input log file>"), cl::init(""), cl::Hidden);

static cl::opt<double>
Threshold("use-asapL1", cl::desc("Fraction of dynamic checks to be preserved"), cl::init(-1.0));

bool DynPassL1::runOnModule(Module &m) {
    ...
    errs() << "DynPassL1 on "<<filename << ";" << Twine(InputSCOV).str().c_str() << "\n";
...
bool DynPassL1::findPhiInst(Instruction *UC_Inst, Instruction *SC_Inst, uint64_t id1, uint64_t id2) {
...
void DynPassL1::optimizeCheckAway(Instruction *Inst) {
...
void DynPassL1::getAnalysisUsage(AnalysisUsage& AU) const {
...
char DynPassL1::ID = 0;
static RegisterPass<DynPassL1> X("DynPassL1",
