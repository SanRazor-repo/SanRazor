...
class SROptimizingCompiler < BaseCompiler
  def do_compile(cmd)
    ...
    if target_name and target_name.end_with?('.o')
      ...
      run!(find_opt(), '-load', 'SRPass.so', "-DynPass#{state.san_level}", "-scov#{state.san_level}=#{scov_name}", "-ucov#{state.san_level}=#{ucov_name}", "-log#{state.san_level}=#{log_name}", "-use-asap#{state.san_level}=#{threshold}", "-o", sr_name, orig_name)
      run!(find_opt(), '-load', 'SRPass.so', "-DynPass#{state.san_level}", "-scov#{state.san_level}=#{scov_name}", "-ucov#{state.san_level}=#{ucov_name}", "-log#{state.san_level}=#{log_name}", "-use-asap#{state.san_level}=#{threshold}", "-o", srbc_name, orig_name)
